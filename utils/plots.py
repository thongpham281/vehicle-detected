...
class Annotator:
    ...
    def box_label(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        ...
        if self.pil or not is_ascii(label):
            ...
            cv2.circle(self.im, ((c1[0] + c2[0]) // 2, (c1[1] + c2[1]) // 2), 4, (0, 255, 0), thickness=self.lw)
    ...
    def box_label_new(self, box, label='', color=(128, 128, 128), txt_color=(255, 255, 255)):
        c1 = (int(box[0]), int(box[1]))
        c2 = (int(box[0]) + int(box[2]), int(box[1]) + int(box[3]))

        cv2.rectangle(self.im, c1, c2, color, thickness=self.lw, lineType=cv2.LINE_AA)
        cv2.circle(self.im, ((c1[0] + c2[0]) // 2, (c1[1] + c2[1]) // 2), 3, (0, 255, 0), thickness=self.lw)

        if label:
            tf = max(self.lw - 1, 1)  # font thickness
            w, h = cv2.getTextSize(label, 0, fontScale=self.lw / 5, thickness=tf)[0]
            c2 = c1[0] + w, c1[1] - h - 3
            # cv2.rectangle(self.im, c1, c2, color, -1, cv2.LINE_4)  # filled
            # cv2.putText(self.im, label, (c1[0], c1[1] - 2), 0, self.lw / 5, txt_color, thickness=tf,
            #             lineType=cv2.LINE_4)
